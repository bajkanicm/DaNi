...
if(isset($_POST['action']) && !empty($_POST['action'])) {
	...
	elseif($action == 'get_all_order_parfume_users'){
		...
		$sql = "SELECT K.id,K.ime,K.firma,K.nadimak,K.problematican,
								(VKS.pocetnostanje+VKS.porudzbinastanje-VKS.uplatastanje) AS dug,								(SELECT 								CASE 								WHEN sledecauplata>'2013-01-01' THEN DATE_FORMAT(sledecauplata,'%d-%m-%Y') 								ELSE 'Nema' END FROM korisnik_uplate 								WHERE korisnik_uplate.idkorisnika= K.id 								AND korisnik_uplate.id = (SELECT MAX(id) FROM korisnik_uplate WHERE korisnik_uplate.idkorisnika= K.id)) 								AS sledecauplata
								FROM porudzbina P
