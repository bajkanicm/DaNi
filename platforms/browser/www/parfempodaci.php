		 ...
		 case 'get_parfume_type' :
			...
			while($row = mysqli_fetch_array($result))
			  ...
			  $data[] = $row;
			   ...
			   echo(json_encode($data));
			break;		case 'get_parfume_type_status' :						$sql="SELECT id, vrsta,				(SELECT COALESCE(SUM(nalageru),0) FROM parfem WHERE parfem.idvrsta=parfem_vrsta.id) AS nalageru				FROM parfem_vrsta ";			$result = mysqli_query($con,$sql);			$data = array();			while($row = mysqli_fetch_array($result))			  {			  $data[] = $row;			  }			   echo(json_encode($data));			break;
